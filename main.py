...
from PIL import Image, ImageFont, ImageOps, ImageDraw,ImageFile
...
API_ENDPOINT = "https://rajaferry.uih.co.th/fb/fbregister.php"
FONT = '/smartcard/THSarabunNew.ttf'
LOGO_IMG = "/smartcard/logo-uih2-ribbon.png"
...
while True:
    print("Waiting for ID card...")
    cardtype = AnyCardType()
    cardrequest = CardRequest(newcardonly = True, timeout= None, cardType=cardtype )
    ...
    fphoto = open("/smartcard/photo.jpeg", "wb")
    for d in PHOTO:
            ...
            if sw1 == 0x61:
                ...
                for i in response:
                    if tis620encoding[i] == '#':
                        ...
                        if  currentcount>0 and currentcount<=1 :
                            firstname = firstname+ tis620encoding[i] 
                            result = result + tis620encoding[i]    
                        elif currentcount>1:
                            lastname = lastname+ tis620encoding[i] 
                            result = result + tis620encoding[i]      
                        else:
                            result = result + tis620encoding[i] 
                result += "\n"
    ...
    printdata = 'Username :' + y['username'] +'\nPassword : '+y['password']
    result = result + printdata
    print("The pastebin URL is:%s"%pastebin_url) 
    p = Usb(0x0483, 0x070b, 0,0x81,0x02)
    p.charcode("MULTILINGUAL")
    font = ImageFont.truetype(FONT, 36)
    profilepic = Image.open(LOGO_IMG)
    profilepic = profilepic.resize((400,100), Image.ANTIALIAS)
    box = font.getsize(result)         # work out size of text
    # make an image same width as text, but twice the height
    print("width : ",box[0])
    print("height : ",box[1])
    image = Image.new('RGB', (600, 10 * box[1]))
    
    draw = ImageDraw.Draw(image)
    # draw the text at the left edge of the box
    draw.text((0, 110), result, font=font)

    x,y = profilepic.size
    image = ImageOps.invert(image) 
    image.paste(profilepic,(-10, 10),profilepic)
    image.save('out1.bmp')
